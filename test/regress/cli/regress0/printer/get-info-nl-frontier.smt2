...
; atom, and each argument of one, names its hosts: the tagged input term
; applying that operation, or a wrapper the input defines as it, to exactly
; its operands, and the instantiations producing such a term, with how many
; there were. Arguments carry hosts of their own because the rewriter can
; leave an atom no input term applies around an argument the input did write. A bound is
