...
; EXPECT: (error "speculate: :instantiate names the variable i twice")
; EXPECT: (error "speculate: :trigger names the variable i twice")
; EXPECT: unknown
; EXPECT: (:speculation (:active false :rounds 4 :loop-threshold 5 :hypotheses () :loops ()))
; EXPECT: unsat
; A term that does not parse, a malformed fingerprint, or a variable named
; twice fails the command with an error and leaves nothing active; the
; session goes on.
(set-logic ALL)
...
(speculate :instantiate ax_f ((i "a") (i "a")))
(speculate :trigger ax_f ((i Int) (i Int)) ("(f i)"))
(assert (< (f a) 0))
(check-sat)
