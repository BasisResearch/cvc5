; COMMAND-LINE: --no-interactive --produce-models --quant-dsplit=none --no-cbqi --user-pat=strict --proof-mode=pp-only --dump-instantiations -o assert-sources
; DISABLE-TESTER: unsat-core
; DISABLE-TESTER: dump
; DISABLE-TESTER: proof
; SCRUBBER: grep -E "^(unsat|sat|unknown)$|^\(\(|^\(\)$|^\(instantiation-sources (user_|prelude_fuel)"
; EXPECT: unsat
; EXPECT: ()
; EXPECT: unsat
; EXPECT: (instantiation-sources prelude_fuel_defaults (?))
; EXPECT: (instantiation-sources user_fixture__ax_f_nonneg_2 (ax_fixture__ax_f_nonneg))
; EXPECT: (instantiation-sources user_fixture__check_4 (hyp_2))
; EXPECT: ((ax_fixture__ax_f_nonneg) (hyp_0) (hyp_2) (query_check hyp_1 ?) (query_check))
; EXPECT: unsat
; EXPECT: ((ax_fixture__ax_f_nonneg))
; A whole Verus query as Verus emits it (s1-evidence/fixture.rs through the
; BasisResearch fork at c08ba92b2d), with :assert-id on the broadcast axiom,
; the three requires hypotheses and the negated query, and
; (get-assertion-sources :tags-only) after each check-sat. Provenance must
; survive the prelude, fuel guards, and the y := f(x) substitution.
(set-logic ALL)
...
(declare-const fuel%fixture!ax_f_nonneg. FuelId)
...
(assert
 (distinct fuel%vstd!function.axiom_fn_mut_call_requires. fuel%vstd!function.axiom_fn_mut_call_ensures.
  fuel%fixture!ax_f_nonneg. fuel%vstd!array.group_array_axioms. fuel%vstd!function.group_function_axioms.
  fuel%vstd!imap.group_imap_lemmas. fuel%vstd!iset.group_iset_lemmas. fuel%vstd!laws_cmp.group_laws_cmp.
...
;; Function-Decl fixture::f
(declare-fun fixture!f.? (Poly) Int)

;; Function-Decl fixture::g
(declare-fun fixture!g.? (Poly) Int)
...
;; Function-Specs fixture::ax_f_nonneg
(declare-fun ens%fixture!ax_f_nonneg. (Int) Bool)
(assert
 (forall ((i! Int)) (!
   (= (ens%fixture!ax_f_nonneg. i!) (>= (fixture!f.? (I i!)) 0))
   :pattern ((ens%fixture!ax_f_nonneg. i!))
   :qid internal_ens__fixture!ax_f_nonneg._definition
)))

;; Function-Def fixture::ax_f_nonneg
;; fixture.rs:12:1: 12:43 (#0)
(push)
 (declare-const i! Int)
 (assert
  fuel_defaults
...
;; Broadcast fixture::ax_f_nonneg
(assert (!
 (=>
  (fuel_bool fuel%fixture!ax_f_nonneg.)
  (forall ((i! Poly)) (!
    (=>
     (has_type i! INT)
     (>= (fixture!f.? i!) 0)
    ...
    :pattern ((fixture!f.? i!))
    :qid user_fixture__ax_f_nonneg_2
))) :assert-id ax_fixture__ax_f_nonneg))

;; Function-Specs fixture::check
(declare-fun req%fixture!check. (Int Int) Bool)
(declare-const %%global_location_label%%0 Bool)
(declare-const %%global_location_label%%1 Bool)
(declare-const %%global_location_label%%2 Bool)
(assert
 (forall ((x! Int) (y! Int)) (!
   (= (req%fixture!check. x! y!) (and
     (=>
     ...
     (=>
      ...
      (forall ((j$ Poly)) (!
        ...
        :pattern ((fixture!g.? j$))
        :qid user_fixture__check_3
   )))))
   :pattern ((req%fixture!check. x! y!))
   :qid internal_req__fixture!check._definition
)))

;; Function-Def fixture::check
;; fixture.rs:19:1: 19:35 (#0)
(push)
 (declare-const x! Int)
 (declare-const y! Int)
...
;; Function-Def fixture::main
;; fixture.rs:36:1: 36:10 (#0)
(push)
